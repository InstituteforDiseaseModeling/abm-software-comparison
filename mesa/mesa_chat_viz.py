...
class SEIRSModel(Model):
    def __init__(self, N, num_nodes, width, height):
        ...
        self.server = ModularServer(
            SEIRSModel,
            [CanvasGrid(agent_portrayal, width, height, 500, 500)],
            "SEIRS Model",
            {"N": N, "num_nodes": num_nodes, "width": width, "height": height},
        )
...
model.server.port = 8521  # Set the server port
model.server.launch()  # This will run the server with real-time visualization

# The simulation will be accessible at http://127.0.0.1:8521/ in your web browser
